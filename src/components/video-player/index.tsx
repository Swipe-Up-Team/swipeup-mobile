...
} from '../../actions/watchVideosActions'
...
class VideoPlayer extends Component {
  ...
  onReadyForDisplay({ naturalSize, status }) {
    ...
    const hours =
      Math.floor(maxSeconds / 3600) >= 10
        ? Math.floor(maxSeconds / 3600)
        : `0${Math.floor(maxSeconds / 3600)}`
    const minutes =
      Math.floor((maxSeconds - hours * 3600) / 60) >= 10
        ? Math.floor((maxSeconds - hours * 3600) / 60)
        : `0${Math.floor((maxSeconds - hours * 3600) / 60)}`
    const minutes2 =
      Math.floor(maxSeconds / 60) >= 10
        ? Math.floor(maxSeconds / 60)
        : `0${Math.floor(maxSeconds / 60)}`
    const second =
      maxSeconds - hours * 3600 - minutes * 60 >= 10
        ? maxSeconds - hours * 3600 - minutes * 60
        : `0${maxSeconds - hours * 3600 - minutes * 60}`
    const second2 =
      maxSeconds - minutes2 * 60 >= 10
        ? maxSeconds - minutes2 * 60
        : `0${maxSeconds - minutes2 * 60}`
    const maxTimeString =
      maxSeconds >= 3600 ? `${hours}:${minutes}:${second}` : `${minutes2}:${second2}`
    this.setState({
      ...this.state,
  ...
  onPlaybackStatusUpdateHandler(status) {
    if (
      this._currentPositionMillis >= this._maxPositionMillis &&
      this._currentPositionMillis !== 0 &&
      !this._didFinished
    ) {
  ...
  onGestureEventHandler({ nativeEvent }) {
    ...
    let nextPositionMillis =
      this._startDraggingPosition + (translationX / maxTimeBarWidth) * this._maxPositionMillis
    nextPositionMillis =
      nextPositionMillis < 0
        ? 0
        : nextPositionMillis > this._maxPositionMillis
        ? this._maxPositionMillis
        : nextPositionMillis
  ...
  render() {
    ...
    const fixedVideoHeight = videoSize.hasOwnProperty('height')
      ? (SCREEN_WIDTH / videoSize.width) * videoSize.height
      : 0
            ...
            >
              ...
              <View style={styles.videoToolBar}>
                <View style={styles.timeBar}>
                  ...
                  <TouchableOpacity
                    onPress={this.onPressTimeBarHandler.bind(this)}
                    activeOpacity={1}
                  ...
                  >
                    <TouchableOpacity activeOpacity={1}>
                      ...
                      >
                        <Animated.View
                          style={{ ...styles.btnTimeControl, left: this._offsetXTimePoint }}
                        ></Animated.View>
                      </PanGestureHandler>
                    </TouchableOpacity>
                    <Animated.View
                      style={{ ...styles.playedBar, width: this._offsetXTimePoint }}
                    ></Animated.View>
                  </TouchableOpacity>
                  <Text style={styles.maxTime}>{maxTimeString}</Text>
                </View>
                <TouchableOpacity
                  onPress={this.onPressOptionIconHandler.bind(this)}
                  style={styles.btnSetting}
                >
                  <FontAwesome5Icon name="cog" color="#fff" size={20}></FontAwesome5Icon>
                </TouchableOpacity>
              </View>
            </View>
          </Animated.View>
...
const mapDispatchToProps = (dispatch, props) => {
  return {
    setCurrentWatchingPosition: (position, videoId) =>
      dispatch(SetCurrentWatchingPosition(position, videoId)),
    setThreadWatchingStatus: (playingId, isPlaying) =>
      dispatch(SetThreadWatchingStatus(playingId, isPlaying))
  }
}
...
const CurrentTimeText = props => {
  ...
  const second =
    maxSeconds - hours * 3600 - minutes * 60 >= 10
      ...
      : `0${maxSeconds - hours * 3600 - minutes * 60}`
  const second2 =
    maxSeconds - minutes2 * 60 >= 10 ? maxSeconds - minutes2 * 60 : `0${maxSeconds - minutes2 * 60}`
  const maxTimeString =
    maxSeconds >= 3600 ? `${hours}:${minutes}:${second}` : `${minutes2}:${second2}`
