...
import { Animated, Image, ImageBackground, LayoutChangeEvent, StyleSheet, Text, View } from "react-native";
...
export default function PopupImage({ selectedPhoto, refProps, _popupImageLocation }: Prop) {
    ...
    const _onAnimatePopup = ({ nativeEvent }: LayoutChangeEvent) => {
        if (selectedPhoto?.content.images) {
            Image.getSize(selectedPhoto?.content.images[0], (xwidth: number, xheight: number) => {
    ...
    return (
        !(selectedPhoto?.content?.images) ? null : <View style={styles.container} >
            <ImageBackground
                ...
                <Animated.View style={{
                    ...
                    <Animated.View style={{
                        ...
                        <ExpoFastImage style={{
                            width: '100%',
                            height: '100%'
                        }} source={{ uri: selectedPhoto.content.images[0], priority: ExpoFastImage.priority.high }}
                        />
                    </Animated.View>
                </Animated.View>
            </ImageBackground>
